...
{
    using Path = CyclicalList<IntPoint>;
    using Paths = List<CyclicalList<IntPoint>>;
    ...
    public class PolyTree : PolyNode
    {
        ...
        public int Total
        {
            get
            {
                ...
                if (result > 0f && m_Childs[0] != m_AllPolys[0]) result--;
                return result;
            }
        }
    ...
    public struct IntPoint
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is IntPoint)
            {
                ...
                //return (X == a.X) && (Y == a.Y);
                return Mathf.Approximately(X, a.X) && Mathf.Approximately(Y, a.Y);
    ...
    public struct IntRect
    {
        ...
        public IntRect(float l, float t, float r, float b)
        {
    ...
    {
        internal const float horizontal = -3.4E+38f;
        internal const int Skip = -2;
        internal const int Unassigned = -1;
        internal const float tolerance = 1.0E-20f;
        internal static bool near_zero(float val) { return (val > -tolerance) && (val < tolerance); }
      ...
      if (!Closed)
        throw new ClipperException("AddPath: Open paths have been disabled.");
            ...
            if (Closed) while (highI > 0f && (pg[highI] == pg[0])) --highI;
            while (highI > 0f && (pg[highI] == pg[highI - 1])) --highI;
        ...
        private void SetDx(TEdge e)
        {
            ...
            if (e.Delta.Y == 0f) e.Dx = horizontal;
      ...
      Swap(ref e.Top.Z, ref e.Bot.Z);
        ...
        public static IntRect GetBounds(Paths paths)
        {
            ...
            while (i < cnt && paths[i].Count == 0f) i++;
            if (i == cnt) return new IntRect(0f, 0f, 0f, 0f);
    ...
    public class Clipper : ClipperBase
    {
      ...
      public ZFillCallback ZFillFunction { get; set; }
        ...
        public Clipper(int InitOptions = 0) : base() //constructor
        {
            ...
            ReverseSolution = (ioReverseSolution & InitOptions) != 0f;
            StrictlySimple = (ioStrictlySimple & InitOptions) != 0f;
            PreserveCollinear = (ioPreserveCollinear & InitOptions) != 0f;
#if use_xyz
          ZFillFunction = null;
#endif
        }
        ...
        {
            //float-linked list: sorted ascending, ignoring dups.
        ...
        private bool ExecuteInternal()
        {
            try
            {
                ...
                foreach (OutRec outRec in m_PolyOuts)
                {
                    ...
                    if ((outRec.IsHole ^ ReverseSolution) == (Area(outRec) > 0f))
      ...
      {
        if (pt.Z != 0f || ZFillFunction == null) return;
      ...
      //------------------------------------------------------------------------------
        ...
        private void InsertLocalMinimaIntoAEL(float botY)
        {
            ...
            while (PopLocalMinima(botY, out lm))
            {
                ...
                if (Op1 != null && IsHorizontal(rb) &&
                  m_GhostJoins.Count > 0f && rb.WindDelta != 0f)
                {
                ...
                if (lb.OutIdx >= 0f && lb.PrevInAEL != null &&
                  //lb.PrevInAEL.Curr.X == lb.Bot.X &&
                  Mathf.Approximately(lb.PrevInAEL.Curr.X, lb.Bot.X) &&
                  lb.PrevInAEL.OutIdx >= 0f &&
                  SlopesEqual(lb.PrevInAEL.Curr, lb.PrevInAEL.Top, lb.Curr, lb.Top, m_UseFullRange) &&
                  lb.WindDelta != 0f && lb.PrevInAEL.WindDelta != 0f)
                {
                ...
                if (lb.NextInAEL != rb)
                {
                    ...
                    if (rb.OutIdx >= 0f && rb.PrevInAEL.OutIdx >= 0f &&
                      SlopesEqual(rb.PrevInAEL.Curr, rb.PrevInAEL.Top, rb.Curr, rb.Top, m_UseFullRange) &&
                      rb.WindDelta != 0f && rb.PrevInAEL.WindDelta != 0f)
                    {
        ...
        private bool E2InsertsBeforeE1(TEdge e1, TEdge e2)
        {
            ...
            if (Mathf.Approximately(e2.Curr.X, e1.Curr.X))
            {
        ...
        private bool IsContributing(TEdge edge)
        {
            ...
            switch (pft)
            {
                case PolyFillType.pftEvenOdd:
                    ...
                    if (edge.WindDelta == 0f && edge.WindCnt != 1) return false;
                    break;
            ...
            switch (m_ClipType)
            {
                ...
                case ClipType.ctXor:
                    if (edge.WindDelta == 0f) //XOr always contributing unless open
                        switch (pft2)
                        {
                            case PolyFillType.pftEvenOdd:
                            case PolyFillType.pftNonZero:
        ...
        private void SetWindingCount(TEdge edge)
        {
            ...
            while (e != null && ((e.PolyTyp != edge.PolyTyp) || (e.WindDelta == 0f))) e = e.PrevInAEL;
            if (e == null)
            {
                PolyFillType pft;
                pft = (edge.PolyTyp == PolyType.ptSubject ? m_SubjFillType : m_ClipFillType);
                if (edge.WindDelta == 0f) edge.WindCnt = (pft == PolyFillType.pftNegative ? -1 : 1);
                else edge.WindCnt = edge.WindDelta;
                edge.WindCnt2 = 0;
                e = m_ActiveEdges; //ie get ready to calc WindCnt2
            }
            else if (edge.WindDelta == 0f && m_ClipType != ClipType.ctUnion)
            {
                edge.WindCnt = 1;
            ...
            else if (IsEvenOddFillType(edge))
            {
                ...
                if (edge.WindDelta == 0)
                {
                    ...
                    while (e2 != null)
                    {
                        if (e2.PolyTyp == e.PolyTyp && e2.WindDelta != 0f)
                            Inside = !Inside;
                        e2 = e2.PrevInAEL;
                    }
            ...
            else
            {
                ...
                if (e.WindCnt * e.WindDelta < 0f)
                {
                    ...
                    if (Math.Abs(e.WindCnt) > 1)
                    {
                        ...
                        if (e.WindDelta * edge.WindDelta < 0f) edge.WindCnt = e.WindCnt;
                        //otherwise continue to 'decrease' WC ...
                        else edge.WindCnt = e.WindCnt + edge.WindDelta;
                    }
                    else
                        //now outside all polys of same polytype so set own WC ...
                        edge.WindCnt = (edge.WindDelta == 0f ? 1 : edge.WindDelta);
                }
                else
                {
                    //prev edge is 'increasing' WindCount (WC) away from zero
                    //so we're inside the previous polygon ...
                    if (edge.WindDelta == 0f)
                        edge.WindCnt = (e.WindCnt < 0f ? e.WindCnt - 1 : e.WindCnt + 1);
                    //if wind direction is reversing prev then use same WC
                    else if (e.WindDelta * edge.WindDelta < 0f)
        ...
        {
            AddOutPt(e1, pt);
            if (e2.WindDelta == 0f) AddOutPt(e2, pt);
        ...
        private OutPt AddLocalMinPoly(TEdge e1, TEdge e2, IntPoint pt)
        {
            ...
            if (prevE != null && prevE.OutIdx >= 0f && prevE.Top.Y < pt.Y && e.Top.Y < pt.Y)
            {
                float xPrev = TopX(prevE, pt.Y);
                float xE = TopX(e, pt.Y);
                if ((xPrev == xE) && (e.WindDelta != 0f) && (prevE.WindDelta != 0f) &&
        ...
        private void SetHoleState(TEdge e, OutRec outRec)
        {
            ...
            while (e2 != null)
            {
                if (e2.OutIdx >= 0f && e2.WindDelta != 0f)
                {
                    if (eTmp == null)
                        eTmp = e2;
        ...
        {
            if (Mathf.Approximately(pt1.Y, pt2.Y)) return horizontal;
        ...
        private bool FirstIsBottomPt(OutPt btmPt1, OutPt btmPt2)
        {
            ...
            float dx1p = Math.Abs(GetDx(btmPt1.Pt, p.Pt));
            p = btmPt1.Next;
            while ((p.Pt == btmPt1.Pt) && (p != btmPt1)) p = p.Next;
            float dx1n = Math.Abs(GetDx(btmPt1.Pt, p.Pt));

            p = btmPt2.Prev;
            while ((p.Pt == btmPt2.Pt) && (p != btmPt2)) p = p.Prev;
            float dx2p = Math.Abs(GetDx(btmPt2.Pt, p.Pt));
            p = btmPt2.Next;
            while ((p.Pt == btmPt2.Pt) && (p != btmPt2)) p = p.Next;
            float dx2n = Math.Abs(GetDx(btmPt2.Pt, p.Pt));

            //if (Math.Max(dx1p, dx1n) == Math.Max(dx2p, dx2n) && Math.Min(dx1p, dx1n) == Math.Min(dx2p, dx2n))
            if (Mathf.Approximately(Math.Max(dx1p, dx1n), Math.Max(dx2p, dx2n)) && Mathf.Approximately(Math.Min(dx1p, dx1n), Math.Min(dx2p, dx2n)))
                return Area(btmPt1) > 0f; //if otherwise identical use orientation
            else
        ...
        private void IntersectEdges(TEdge e1, TEdge e2, IntPoint pt)
        {
            ...
            bool e1Contributing = (e1.OutIdx >= 0f);
            bool e2Contributing = (e2.OutIdx >= 0f);
          ...
          SetZ(ref pt, e1, e2);
            ...
            if (e1.WindDelta == 0f || e2.WindDelta == 0f)
            {
                //ignore subject-subject open path intersections UNLESS they
                //are both open paths, AND they are both 'contributing maximas' ...
                if (e1.WindDelta == 0f && e2.WindDelta == 0f) return;
                ...
                {
                    if ((e1.WindDelta == 0f) && Math.Abs(e2.WindCnt) == 1 &&
                      (m_ClipType != ClipType.ctUnion || e2.WindCnt2 == 0f))
                    {
                        AddOutPt(e1, pt);
                        if (e1Contributing) e1.OutIdx = Unassigned;
                    }
                    else if ((e2.WindDelta == 0f) && (Math.Abs(e1.WindCnt) == 1) &&
                      (m_ClipType != ClipType.ctUnion || e1.WindCnt2 == 0f))
                    {
                        AddOutPt(e2, pt);
                        if (e2Contributing) e2.OutIdx = Unassigned;
                    }
            ...
            if (e1.PolyTyp == e2.PolyTyp)
            {
                ...
                {
                    if (e1.WindCnt + e2.WindDelta == 0f) e1.WindCnt = -e1.WindCnt;
                    else e1.WindCnt += e2.WindDelta;
                    if (e2.WindCnt - e1.WindDelta == 0f) e2.WindCnt = -e2.WindCnt;
            ...
            {
                if ((e1Wc != 0f && e1Wc != 1) || (e2Wc != 0f && e2Wc != 1) ||
            ...
            else if ((e1Wc == 0f || e1Wc == 1) && (e2Wc == 0f || e2Wc == 1))
            {
                ...
                if (e1.PolyTyp != e2.PolyTyp)
                {
                    ...
                    switch (m_ClipType)
                    {
                        ...
                        case ClipType.ctUnion:
                            ...
                            break;
                        case ClipType.ctDifference:
                            if (((e1.PolyTyp == PolyType.ptClip) && (e1Wc2 > 0f) && (e2Wc2 > 0f)) ||
                                ((e1.PolyTyp == PolyType.ptSubject) && (e1Wc2 <= 0f) && (e2Wc2 <= 0f)))
                                AddLocalMinPoly(e1, e2, pt);
                            break;
                        case ClipType.ctXor:
                            AddLocalMinPoly(e1, e2, pt);
        ...
        private void ProcessHorizontal(TEdge horzEdge)
        {
            ...
            bool IsOpen = horzEdge.WindDelta == 0f;
            ...
            for (; ; ) //loop through consec. horizontal edges
            {
                ...
                while (e != null)
                {
                    ...
                    if (Mathf.Approximately(e.Curr.X, horzEdge.Top.X) && horzEdge.NextInLML != null &&
                    e.Dx < horzEdge.NextInLML.Dx) break;

                    if (horzEdge.OutIdx >= 0f && !IsOpen)  //note: may be done multiple times
                    {
#if use_xyz
                ...
                if (horzEdge.OutIdx >= 0f) AddOutPt(horzEdge, horzEdge.Bot);
                GetHorzDirection(horzEdge, out dir, out horzLeft, out horzRight);

            } //end for (;;)

            if (horzEdge.OutIdx >= 0f && op1 == null)
            {
            ...
            if (horzEdge.NextInLML != null)
            {
                if (horzEdge.OutIdx >= 0f)
                {
                    ...
                    UpdateEdgeIntoAEL(ref horzEdge);
                    if (horzEdge.WindDelta == 0f) return;
                    //nb: HorzEdge is no longer horizontal here
                    TEdge ePrev = horzEdge.PrevInAEL;
                    TEdge eNext = horzEdge.NextInAEL;
                    //if (ePrev != null && ePrev.Curr.X == horzEdge.Bot.X &&
                    //  ePrev.Curr.Y == horzEdge.Bot.Y && ePrev.WindDelta != 0f &&
                    if (ePrev != null && ePrev.Curr.X == horzEdge.Bot.X &&
  ePrev.Curr.Y == horzEdge.Bot.Y && ePrev.WindDelta != 0f &&

  (ePrev.OutIdx >= 0f && ePrev.Curr.Y > ePrev.Top.Y &&
  SlopesEqual(horzEdge, ePrev, m_UseFullRange)))
                    {
                      ...
                      eNext.Curr.Y == horzEdge.Bot.Y && eNext.WindDelta != 0f &&
                      eNext.OutIdx >= 0f && eNext.Curr.Y > eNext.Top.Y &&
            ...
            {
                if (horzEdge.OutIdx >= 0f) AddOutPt(horzEdge, horzEdge.Top);
                DeleteFromAEL(horzEdge);
            }
        }
        //------------------------------------------------------------------------------
        ...
        private bool ProcessIntersections(float topY)
        {
            ...
            try
            {
                ...
                if (m_IntersectList.Count == 0f) return true;
        ...
        {
            return value;// value < 0f ? (cInt)(value - 0f.5) : (cInt)(value + 0f.5);
        }
        //------------------------------------------------------------------------------
        ...
        private void ProcessEdgesAtTopOfScanbeam(float topY)
        {
            ...
            while (e != null)
            {
              ...
              if (e.Top.Y == topY) e.Curr.Z = e.Top.Z;
                    ...
                    {
                        TEdge ePrev = e.PrevInAEL;
                        if ((e.OutIdx >= 0f) && (e.WindDelta != 0f) && ePrev != null &&
                          (ePrev.OutIdx >= 0f) && (ePrev.Curr.X == e.Curr.X) &&
                          (ePrev.WindDelta != 0f))
                        {
            ...
            while (e != null)
            {
                if (IsIntermediate(e, topY))
                {
                    ...
                    if (ePrev != null && ePrev.Curr.X == e.Bot.X &&
                      ...
                      (e.WindDelta != 0f) && (ePrev.WindDelta != 0f))
                    {
                      ...
                      (e.WindDelta != 0f) && (eNext.WindDelta != 0f))
                    {
        ...
        private void DoMaxima(TEdge e)
        {
            ...
            else if (e.OutIdx >= 0f && eMaxPair.OutIdx >= 0f)
            {
                if (e.OutIdx >= 0f) AddLocalMaxPoly(e, eMaxPair, e.Top);
                DeleteFromAEL(e);
                DeleteFromAEL(eMaxPair);
            }
#if use_lines
            else if (e.WindDelta == 0f)
            {
                if (e.OutIdx >= 0f)
                {
                    AddOutPt(e, e.Top);
        ...
        {
            //returns 0f if false, +1 if true, -1 if pt ON polygon boundary
            ...
            for (int i = 1; i <= cnt; ++i)
            {
                ...
                //if (ipNext.Y == pt.Y)
                if (Mathf.Approximately(ipNext.Y, pt.Y))
                ...
                if ((ip.Y < pt.Y) != (ipNext.Y < pt.Y))
                {
                    if (ip.X >= pt.X)
                    {
                        ...
                        else
                        {
                            ...
                            //if (d == 0f) return -1;
                            if (Mathf.Approximately(d, 0f)) return -1;
                            else if ((d > 0f) == (ipNext.Y > ip.Y)) result = 1 - result;
                        }
                    }
                    else
                    {
            ...
            do
            {
                ...
                if ((poly0y < pty) != (poly1y < pty))
                {
                    if (poly0x >= ptx)
                    {
                        ...
                        {
                            float d = (float)(poly0x - ptx) * (poly1y - pty) -
                              (float)(poly1x - ptx) * (poly0y - pty);
                            //if (d == 0f) return -1;
                            if (Mathf.Approximately(d, 0f)) return -1;
                            if ((d > 0f) == (poly1y > poly0y)) result = 1 - result;
                        }
                    }
                    else
                    {
        ...
        private static bool Poly2ContainsPoly1(OutPt outPt1, OutPt outPt2)
        {
            ...
            do
            {
                //nb: PointInPolygon returns 0f if false, +1 if true, -1 if pt on polygon
                int res = PointInPolygon(op.Pt, outPt2);
                if (res >= 0f) return res > 0f;
                op = op.Next;
            }
            while (op != outPt1);
            return true;
            ...
            for (int i = 0; i < m_Joins.Count; i++)
            {
                ...
                if (outRec1 == outRec2)
                {
                    ...
                    if (Poly2ContainsPoly1(outRec2.Pts, outRec1.Pts))
                    {
                        ...
                        if ((outRec2.IsHole ^ ReverseSolution) == (Area(outRec2) > 0f))
                    ...
                    else if (Poly2ContainsPoly1(outRec1.Pts, outRec2.Pts))
                    {
                        ...
                        if ((outRec1.IsHole ^ ReverseSolution) == (Area(outRec1) > 0f))
        ...
        public static float Area(Path poly)
        {
            ...
            {
                a += (poly[j].X + poly[i].X) * ((float)poly[j].Y - poly[i].Y);
                j = i;
            }
            return -a * 0.5f;
        }
        ...
        internal float Area(OutRec outRec)
        {
            return Area(outRec.Pts);
        }
        //------------------------------------------------------------------------------

        internal float Area(OutPt op)
        {
            OutPt opFirst = op;
            ...
            do
            {
                a = a + (float)(op.Prev.Pt.X + op.Pt.X) * (float)(op.Prev.Pt.Y - op.Pt.Y);
                op = op.Next;
            } while (op != opFirst);
            return a * 0.5f;
        }
        ...
        {
            //The equation of a line in general form (Ax + By + C = 0f)
            //given 2 points (x¹,y¹) & (x²,y²) is ...
            //(y¹ - y²)x + (x² - x¹)y + (y² - y¹)x¹ - (x² - x¹)y¹ = 0f
        ...
        private static bool SlopesNearCollinear(IntPoint pt1,
            IntPoint pt2, IntPoint pt3, float distSqrd)
        {
    ...
    public class ClipperOffset
    {
        ...
        private const float two_pi = UnityEngine.Mathf.PI * 2f;
        ...
        public void AddPath(Path path, JoinType joinType, EndType endType)
        {
            ...
            if (endType == EndType.etClosedLine || endType == EndType.etClosedPolygon)
                while (highI > 0f && path[0] == path[highI]) highI--;
            ...
            for (int i = 1; i <= highI; i++)
                if (newNode.m_polygon[j] != path[i])
                {
                    ...
                    if (path[i].Y > newNode.m_polygon[k].Y ||
                      ...
                      (Mathf.Approximately(path[i].Y, newNode.m_polygon[k].Y) &&
            ...
            else
            {
                ...
                if (newNode.m_polygon[k].Y > ip.Y ||
                  ...
                  (Mathf.Approximately(newNode.m_polygon[k].Y, ip.Y) &&
        ...
        internal static Vector2 GetUnitNormal(IntPoint pt1, IntPoint pt2)
        {
            float dx = (pt2.X - pt1.X);
            float dy = (pt2.Y - pt1.Y);
            //if ((dx == 0f) && (dy == 0f)) return new Vector2(0f, 0f);
            if (Mathf.Approximately(dx, 0f) && Mathf.Approximately(dy, 0f)) return new Vector2(0f, 0f);

            float f = 1 * 1.0f / UnityEngine.Mathf.Sqrt(dx * dx + dy * dy);
            dx *= f;
            dy *= f;
        ...
        private void DoOffset(float delta)
        {
            ...
            float steps = UnityEngine.Mathf.PI / Mathf.Acos(1 - y / Math.Abs(delta));
            m_sin = Mathf.Sin(two_pi / steps);
            m_cos = Mathf.Cos(two_pi / steps);
            ...
            for (int i = 0; i < m_polyNodes.ChildCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        int j = len - 1;
                        pt1 = new IntPoint(Round(m_srcPoly[j].X + m_normals[j].x * delta), Round(m_srcPoly[j].Y + m_normals[j].y * delta));
                        m_destPoly.Add(pt1);
                        pt1 = new IntPoint(Round(m_srcPoly[j].X - m_normals[j].x * delta), Round(m_srcPoly[j].Y - m_normals[j].y * delta));
                        m_destPoly.Add(pt1);
                    }
                    ...
                    //re-build m_normals ...
                    for (int j = len - 1; j > 0f; j--)
                        m_normals[j] = -m_normals[j - 1];

                    m_normals[0] = -m_normals[1];

                    k = len - 1;
                    for (int j = k - 1; j > 0f; --j)
                        OffsetPoint(j, ref k, node.m_jointype);
        ...
        public void Execute(ref Paths solution, float delta)
        {
            ...
            else
            {
                ...
                if (solution.Count > 0f) solution.RemoveAt(0);
            }
        }
        //------------------------------------------------------------------------------
        ...
        public void Execute(ref PolyTree solution, float delta)
        {
            ...
            else
            {
                ...
                if (solution.ChildCount == 1 && solution.Childs[0].ChildCount > 0f)
                {
            ...
            if (m_sinA * m_delta < 0f)
            {
                ...
                switch (jointype)
                {
                    case JoinType.jtMiter:
                        {
                            float r = 1 + (m_normals[j].x * m_normals[k].x + m_normals[j].y * m_normals[k].y);
        ...
        {
            float dx = UnityEngine.Mathf.Tan(UnityEngine.Mathf.Atan2(m_sinA, m_normals[k].x * m_normals[j].x + m_normals[k].y * m_normals[j].y) / 4);
        ...
        {
            float a = UnityEngine.Mathf.Atan2(m_sinA, m_normals[k].x * m_normals[j].x + m_normals[k].y * m_normals[j].y);
