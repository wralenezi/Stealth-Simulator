...
public static class IListInsertIntoSortedListExtensions
{
    ...
    public static void InsertIntoSortedList<T>(this IList<T> list, T value, Order order)
        where T : IComparable<T>
    {
        InsertIntoSortedList(list, value, (a, b) => a.CompareTo(b), order);
    }
    ...
    public static void InsertIntoSortedList<T>(
        this IList<T> list,
        T value,
        Comparison<T> comparison, Order order
    )
    {
        var startIndex = 0;
