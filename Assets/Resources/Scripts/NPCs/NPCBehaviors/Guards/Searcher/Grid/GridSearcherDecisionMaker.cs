...
public class GridSearcherDecisionMaker
{
    private Dictionary<string, Vector2> _guardGoals;

    public GridSearcherDecisionMaker()
    {
        _guardGoals = new Dictionary<string, Vector2>();
    }

    public void SetTarget(Guard guard, List<Guard> guards, GridSearcherParams _params, List<Node> _nodes)
    {
        Vector2? target = null;
    ...
    private Vector2? GetWeightedSumFittestTarget(Guard guard, List<Guard> guards, GridSearcherParams _params,
        List<Node> _node)
    {
